define("react", ['exports'], (function (exports) { 'use strict';
  ...
  function elementRefGetterWithDeprecationWarning() {
    {
      ...
      if (!didWarnAboutElementRef[componentName]) {
        ...
        console.warn('Accessing element.ref was removed in React 19. ref is now a ' + 'regular prop. It will be removed from the JSX Element ' + 'type in a future release.');
      }
  ...
  function cloneAndReplaceKey(oldElement, newKey) {
    const clonedElement = ReactElement(oldElement.type, newKey, oldElement.props, oldElement._owner, oldElement._debugStack, oldElement._debugTask);
    {
  ...
  function cloneElement(element, config, children) {
    ...
    const clonedElement = ReactElement(element.type, key, props, owner, element._debugStack, element._debugTask);
    for (let i = 2; i < arguments.length; i++) {
      validateChildKeys(arguments[i]);
    }
    return clonedElement;
  ...
  function act(callback) {
    {
      ...
      const prevIsBatchingLegacy = false;
      ...
      try {
        ...
        result = callback();
        const didScheduleLegacyUpdate = !disableLegacyMode ? ReactSharedInternals.didScheduleLegacyUpdate : false;
      ...
      typeof result.then === 'function') {
        ...
        queueSeveralMicrotasks(() => {
          if (!didAwaitActCall && !didWarnNoAwaitAct) {
            ...
            // react-19-enable error to warn
            console.warn('You called act(async () => ...) without await. ' + 'This could lead to unexpected testing behaviour, ' + 'interleaving multiple act calls and mixing their ' + 'scopes. ' + 'You should - await act(async () => ...);');
          }
        });
        return {
          then(resolve, reject) {
