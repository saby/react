define("react-dom/server.node", ['exports', 'util', 'crypto', 'async_hooks', 'react', 'react-dom', 'stream'], (function (exports, util, crypto, async_hooks, React, ReactDOM, stream) { 'use strict';
  ...
  var reactDOMPackageVersion = '19.1.0';
  ...
  function pushStartObject(target, props) {
  ...
  function pushImg(target, props, resumableState, renderState, pictureOrNoScriptTagInScope) {
    const src = props.src,
      srcSet = props.srcSet;
    if (props.loading !== 'lazy' && (src || srcSet) && (typeof src === 'string' || src == null) && (typeof srcSet === 'string' || srcSet == null) && props.fetchPriority !== 'low' && pictureOrNoScriptTagInScope === false &&
  ...
  function pushStartHead(target, props, renderState, preambleState, insertionMode) {
  ...
  function pushStartBody(target, props, renderState, preambleState, insertionMode) {
    if (insertionMode < HTML_MODE) {
  ...
  function pushStartHtml(target, props, renderState, preambleState, insertionMode) {
  ...
  function pushStartInstance(target, type, props, resumableState, renderState, preambleState, hoistableState, formatContext, textEmbedded, isFallback) {
    {
    ...
    switch (type) {
      ...
      // Preamble start tags
      case 'head':
        return pushStartHead(target, props, renderState, preambleState, formatContext.insertionMode);
      case 'body':
        return pushStartBody(target, props, renderState, preambleState, formatContext.insertionMode);
      case 'html':
        {
          return pushStartHtml(target, props, renderState, preambleState, formatContext.insertionMode);
        }
      default:
        {
          if (type.indexOf('-') !== -1) {
  ...
  function writeEndCompletedSuspenseBoundary(destination, renderState, preambleState) {
    if (preambleState) {
  ...
  function writeEndClientRenderedSuspenseBoundary(destination, renderState, preambleState) {
    if (preambleState) {
      writePreambleContribution(destination, preambleState);
    }
    return writeChunkAndReturn(destination, endSuspenseBoundary);
  ...
  function writePreambleStart(destination, resumableState, renderState, willFlushAllSegments) {
    const preamble = renderState.preamble;
  ...
  function createRequest(children, resumableState, renderState, rootFormatContext, progressiveChunkSize, onError, onAllReady, onShellReady, onShellError, onFatalError, onPostpone, formState) {
    ...
    const rootTask = createRenderTask(request, null, children, -1, null, rootSegment, null, null, request.abortableTasks, null, rootFormatContext, rootContextSnapshot, emptyTreeContext, null, false, emptyContextObject, null);
  ...
  function resumeRequest(children, postponedState, renderState, onError, onAllReady, onShellReady, onShellError, onFatalError, onPostpone) {
    ...
    if (typeof postponedState.replaySlots === 'number') {
      ...
      // There is no parent so conceptually, we're unblocked to flush this segment.
      rootSegment.parentFlushed = true;
      const rootTask = createRenderTask(request, null, children, -1, null, rootSegment, null, null, request.abortableTasks, null, postponedState.rootFormatContext, rootContextSnapshot, emptyTreeContext, null, false, emptyContextObject, null);
      pushComponentStack(rootTask);
      request.pingedTasks.push(rootTask);
      return request;
    }
    ...
    const rootTask = createReplayTask(request, null, replay, children, -1, null, null, request.abortableTasks, null, postponedState.rootFormatContext, rootContextSnapshot, emptyTreeContext, null, false, emptyContextObject, null);
    pushComponentStack(rootTask);
    request.pingedTasks.push(rootTask);
    return request;
  }
  ...
  function renderSuspenseBoundary(request, someTask, keyPath, props) {
    ...
    if (request.trackedPostpones !== null) {
      ...
      const suspendedPrimaryTask = createRenderTask(request, null, content, -1, newBoundary, contentRootSegment, newBoundary.contentPreamble, newBoundary.contentState, task.abortSet, keyPath, task.formatContext, task.context, task.treeContext, task.componentStack, task.isFallback, emptyContextObject, task.debugTask );
      ...
      const suspendedFallbackTask = createRenderTask(request, null, fallback, -1, parentBoundary, boundarySegment, newBoundary.fallbackPreamble, newBoundary.fallbackState, fallbackAbortSet, fallbackKeyPath, task.formatContext, task.context, task.treeContext, task.componentStack, true, emptyContextObject, task.debugTask );
  ...
  function replaySuspenseBoundary(request, task, keyPath, props, id, childNodes, childSlots, fallbackNodes, fallbackSlots) {
    ...
    const suspendedFallbackTask = createReplayTask(request, null, fallbackReplay, fallback, -1, parentBoundary, resumedBoundary.fallbackState, fallbackAbortSet, fallbackKeyPath, task.formatContext, task.context, task.treeContext, task.componentStack, true, emptyContextObject, task.debugTask );
    pushComponentStack(suspendedFallbackTask);
    // TODO: This should be queued at a separate lower priority queue so that we only work
    // on preparing fallbacks if we don't have any more main content to task on.
    request.pingedTasks.push(suspendedFallbackTask);
  ...
  function renderPreamble(request, task, blockedSegment, node) {
    ...
    const preambleTask = createRenderTask(request, null, node, -1, task.blockedBoundary, preambleSegment, task.blockedPreamble, task.hoistableState, request.abortableTasks, task.keyPath, task.formatContext, task.context, task.treeContext, task.componentStack, task.isFallback, emptyContextObject, task.debugTask );
  ...
  function retryRenderTask(request, task, segment) {
    ...
    try {
      ...
      erroredTask(request, task.blockedBoundary, x, errorInfo, task.debugTask );
      return;
    } finally {
      {
  ...
  function prerenderToNodeStream(children, options) {
    return new Promise((resolve, reject) => {
      ...
      const resumableState = createResumableState(options ? options.identifierPrefix : undefined, options ? options.unstable_externalRuntimeSrc : undefined, options ? options.bootstrapScriptContent : undefined, options ? options.bootstrapScripts : undefined, options ? options.bootstrapModules : undefined);
      const request = createPrerenderRequest(children, resumableState, createRenderState(resumableState, undefined,
  ...
  function resumeAndPrerenderToNodeStream(children, postponedState, options) {
    return new Promise((resolve, reject) => {
      const onFatalError = reject;
      function onAllReady() {
        const readable = new stream.Readable({
      ...
      const request = resumeAndPrerenderRequest(children, postponedState, resumeRenderState(postponedState.resumableState, options ? options.nonce : undefined), options ? options.onError : undefined, onAllReady, undefined, undefined, onFatalError, options ? options.onPostpone : undefined);
      if (options && options.signal) {
        const signal = options.signal;
        if (signal.aborted) {
          abort(request, signal.reason);
  ...
  exports.resumeAndPrerenderToNodeStream = resumeAndPrerenderToNodeStream;
