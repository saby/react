...
define('react-dom/test-utils', ['exports', 'react', 'react-dom'], function (exports, React, ReactDOM) { 'use strict';
  ...
  function act(callback) {
    ...
    function onDone() {
...

      {
        if (actingUpdatesScopeDepth > previousActingUpdatesScopeDepth) {
          ...
          sabyError('react-dom error! ' + 'You seem to have overlapping act() calls, this is not supported. ' + 'Be sure to await previous act() calls before making a new one. ');
        }
      }
    }
    ...
    if (result !== null && typeof result === 'object' && typeof result.then === 'function') {
...

      {
        if (typeof Promise !== 'undefined') {
          ...
          Promise.resolve().then(function () {}).then(function () {
            if (called === false) {
              sabyError('react-dom error! ' + 'You called act(async () => ...) without await. ' + 'This could lead to unexpected testing behaviour, interleaving multiple act ' + 'calls and mixing their scopes. You should - await act(async () => ...);');
            }
          });
        }
      ...
      {
        if (result !== undefined) {
          sabyError('react-dom error! ' + 'The callback passed to act(...) function ' + 'must return undefined, or a Promise. You returned %s', result);
        }
      ...
      return {
        then: function (resolve) {
          {
            sabyError('react-dom error! ' + 'Do not await the result of calling act(...) with sync logic, it is not a Promise.');
          }
  ...
  function accumulateDirectionalDispatches(inst, phase, event) {
    {
      if (!inst) {
        sabyError('react-dom error! ' + 'Dispatching inst must not be null');
      }
    }
